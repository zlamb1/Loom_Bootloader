...
static void loom_outb(loom_u16 port, loom_u8 v) {
    __asm__ volatile("outb %1, %0" :: "Nd"(port), "a"(v));
}

static void loom_vga_sync_cursor(loom_usize x, loom_usize y) {
...
static loom_usize loom_vga_get_x(loom_console *con) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
    return vga_con->x;
}

static loom_usize loom_vga_get_y(loom_console *con) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
    return vga_con->y;
}

static loom_u8 loom_vga_get_fg(loom_console *con) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
    return vga_con->attribs & 0xF;
}

static loom_u8 loom_vga_get_bg(loom_console *con) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
    return (vga_con->attribs >> 4) & 0xF;
}

static loom_error loom_vga_set_x(loom_console *con, loom_usize x) {
    if (x >= ROWS) {
...
static loom_error loom_vga_set_y(loom_console *con, loom_usize y) {
    if (y >= COLS) {
        return LOOM_ERR_BAD_ARG;
    }
...
static loom_error loom_vga_set_fg(loom_console *con, loom_u8 fg) {
...
static loom_error loom_vga_set_bg(loom_console *con, loom_u8 bg) {
...
static void loom_vga_clear(loom_console *con) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
    loom_u16 char_and_attribs = (loom_u16) ' ' | ((loom_u16) vga_con->attribs << 8);
...
static void loom_vga_write(loom_console *con, loom_usize len, const char *buf) {
    loom_vga_console *vga_con = (loom_vga_console *) con->data;
